# DDQ's Cyber Thermometer v0.8.2

中文名：动动枪赛博体温计

本版本补发菜单月历功能到 GitHub Release，让旧版在线升级可以检测到更新。

## 功能

- 顶栏融合小组件：迷你柱显示内存压力、内存和 CPU，右侧显示核心温度。
- 点击顶栏图标后，在“内存压力”下方显示极简月历。
- 月历默认周一开始、周日结束。
- 月历支持切换上个月和下个月。
- 打开菜单时默认回到当前月份，并高亮当天。
...
- 空入口可直接点击添加名称和链接。
- 已有入口可通过“管理快捷入口…”修改或清空。
- 快捷入口链接使用系统默认浏览器打开。
- 下载和上传速率改为上下两行显示，减少状态栏横向占用。
- 网络速率文字改为跟温度数字一致的高对比颜色，避免和背景融在一起。
- 顶栏紧凑流量使用固定三位有效数字显示，减少 `0.10KB`、`10.2KB`、`99.9KB` 等不同数值之间的宽度变化。
- 超过当前单位后自动升到更高单位，例如 `999KB` 显示为 `0.99MB`，`120MB` 显示为 `0.12GB`。
...
下载 `DDQs-Cyber-Thermometer-0.8.2.dmg`，打开后把 `动动枪赛博体温计.app` 拖到 Applications。

Release 同时提供 `DDQs-Cyber-Thermometer-0.8.2.app.zip`，供 App 内在线升级优先使用。
