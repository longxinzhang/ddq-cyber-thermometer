...
App Bundle 元信息。当前 bundle 标识为 `com.dongdongqiang.cyber-thermometer`，展示名为“动动枪赛博体温计”，版本为 `0.8.2`。`LSUIElement` 为 `true`，表示 App 作为菜单栏应用运行，不显示 Dock 图标。

### `Sources/MacHealthGuardian/`

App target，负责菜单栏应用入口和用户界面相关逻辑：

- `App/`：SwiftUI App 入口、`AppDelegate`、快捷入口设置、菜单月历和命令行模式。
- `Rendering/`：状态栏图像绘制。
...
- `RELEASE_NOTES.md`：当前版本发布说明，描述 v0.8.2 的功能和安装信息。
